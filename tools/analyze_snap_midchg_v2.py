...
"""
统计 Snapback 的中观 N 分钟涨跌幅（默认 15mChg）与离场结果的关系。

定义：
    mid_chg_pct = (anchor_price / price_N_minutes_ago - 1) * 100

anchor:
- c:     用 C 点时间 / C 点价格
- entry: 用 entry_time / entry_price

输出：
1) 按 0 分界：<0 / >=0
2) 按细分桶：<-10 / [-10,-5) / [-5,0) / [0,5) / [5,10) / >=10
3) 各桶 trades / win_rate / avg_pnl / TP/SL/TS/BS 占比等

说明：
- 依赖 pandas（你的 /root/service_env/bin/python 已有）
- 自动递归查找 symbol 的 parquet 文件/目录
"""
...
def _candidate_paths(root: Path, symbol: str) -> List[Path]:
    ...
    direct_file = root / f"{symbol}.parquet"
    if direct_file.exists():
        out.append(direct_file)

    direct_dir = root / symbol
    if direct_dir.exists():
        out.append(direct_dir)

    # 常见情形：递归存在 <something>/<symbol>.parquet
    out.extend(sorted(root.rglob(f"{symbol}.parquet")))

    # 常见情形：递归存在目录名就是 symbol，目录里是 parquet 分片
    out.extend(sorted([p for p in root.rglob(symbol) if p.is_dir()]))
...
def _path_looks_like_symbol_dataset(path: Path) -> bool:
...
def _find_parquet(kline_root: Path, symbol: str) -> Path:
    candidates = [p for p in _candidate_paths(kline_root, symbol) if _path_looks_like_symbol_dataset(p)]
    if not candidates:
        raise FileNotFoundError(
            f"未找到 {symbol} 的 parquet 数据。请检查 --kline-root，当前 root={kline_root}"
        )

    # 偏好：
    # 1) 直接文件 root/symbol.parquet
    # 2) 更短路径
    # 3) 字典序
    candidates = sorted(
        candidates,
        key=lambda p: (
            0 if p == (kline_root / f"{symbol}.parquet") else 1,
            len(str(p)),
            str(p),
        ),
    )
...
def _load_symbol_df(cache: Dict[str, pd.DataFrame], kline_root: Path, symbol: str) -> pd.DataFrame:
    ...
    if "open_time_ms" in df.columns:
        ...
        raise RuntimeError(f"{path} 缺少 open_time_ms / open_time / DatetimeIndex")
    ...
    if "close" not in df.columns:
        raise RuntimeError(f"{path} 缺少 close 列")
...
def _extract_midchg_row(
    ...
    return {
        ...
        "drop_pct": _safe_float(_ctx_first(ctx, "drop_pct", "dropPct")),
        "rebound_ratio": _safe_float(_ctx_first(ctx, "rebound_ratio", "reboundRatio")),
    }
