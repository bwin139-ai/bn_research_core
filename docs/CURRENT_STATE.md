...
40. 2026-06-28 MacBook 本地代理混用风险已固定为工具脚本：新增 `tools/mac_proxy/proxy_status.sh`、`tools/mac_proxy/use_aws_proxy.sh`、`tools/mac_proxy/use_monoproxy.sh`。AWS 模式启动本机 SSH SOCKS 隧道 `127.0.0.1:18080 -> ubuntu@13.230.97.189`，并同步 macOS Wi-Fi SOCKS、git 全局代理与 `~/.zshrc` 托管代理块；MonoProxy 模式恢复 `127.0.0.1:8118/8119`。脚本不保存私钥内容，只引用本机 key 路径；该工具只用于 MacBook 开发环境，不得复制到阿里云生产交易进程环境。
