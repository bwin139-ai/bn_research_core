...
TVR recovery ladder 已进入配置/校验、state 预备与 dry-run gate 评估阶段，但当前 live 仍未实现自动 recovery；`recovery.enabled=true` 在非 dry-run 实盘模式会 fail-fast。当前 live 建立单笔 `open_trade` 时会同步写入 `open_lots[0]` 镜像，旧 state 若只有 `open_trade` 会在 reconcile 时补齐镜像；dry-run 可评估 active symbol 是否满足 recovery ladder，后续仍需实现真正多 lot lifecycle，并用 max_symbol_entry_notional_usdt 按本金成本口径限制每 symbol 最大投入。
```
