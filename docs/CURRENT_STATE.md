...
它不复用山寨币三策略的结构语义，第一阶段已建设 data_hub；策略侧第一刀只做 decision audit，不下单。
```
...
- decision_audit.risk.proposed_order_notional_usdt = 1000
- decision_audit.risk.max_symbol_notional_usdt = 1000
- decision_audit.risk.max_total_notional_usdt = 10000
```
...
3. TVR decision audit 可先在服务器常驻运行，观察 selected_intents、拒绝原因和参数敏感性；确认稳定后再另起 patch 接入真实 maker-only 执行层。
