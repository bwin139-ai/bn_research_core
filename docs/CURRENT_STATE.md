...
14. 2026-04-30 已复盘 `HUSDT 18:15 BJ` 在 `junjie2026` / `chen912` 的 `SL fail -> TIME_STOP` 表象：两账户 entry 后提交 SL 均被 Binance 拒绝 `APIError(code=-2021): Order would immediately trigger.`，随后代码进入 `entry_sl_fail_flatten` 风险分支，用 `SNP_TS` / `place_time_stop_order()` 提交 reduce-only market 应急平仓，最终 live projection / 通知显示为 `reason=TIME_STOP`。这在订单 leg 层面来自复用 TS 订单，但语义层面不准确；该类离场应单独定义 exit reason 与 client order leg/custom id，例如 `SL_SUBMIT_FAILED_FLATTEN` 或 `PROTECTIVE_FLATTEN_AFTER_SL_FAIL`，避免混入正常策略 time-stop。现场价格对比：C bar=`2026-04-30 18:14 BJ`，C close=`0.17015`，`C+1m` open=`0.17016`，signal current_price=`0.17015`，resolved SL=`0.16837356`，C close 到 SL 仍有约 `1.06%` 空间；但实际 entry 发生在 `18:15:37 BJ` 左右，`junjie2026` entry=`0.1681068`、`chen912` entry=`0.1679185`，分别已低于 SL 约 `0.16%` / `0.27%`，因此 SL submit 必然被交易所视为立即触发风险。已新增 live pre-entry price guard：在提交市价 entry 前即时读取并落盘 `pre_entry_price`，价格源与 SL `workingType=CONTRACT_PRICE` 对齐；若 `(pre_entry_price - resolved_sl_price) / resolved_sl_price` 小于显式配置阈值，则跳过 entry 并写 audit/stage 记录，避免先开仓再因 SL 失效而应急强平。
...
7. 专门拆刀处理 `entry_sl_fail_flatten` 语义：不得继续混用 `TIME_STOP` 作为离场原因和 custom id；需要新增独立 leg / client order id / live_trade reason / 通知文案，并保持 LONG-only。
8. 部署 live pre-entry SL distance guard 后，观察是否出现 `pre_entry_price_guard_skip`，并确认该类 skip 不再产生 entry / SL fail / emergency flatten 链路。
