...
2. Spring-SABC 两个结构差异样本继续查证：`STORJUSDT 2026-05-16 08:55`、`PROVEUSDT 2026-05-21 16:00`。当前事实是本地 2026-05 parquet 覆盖完整，但 sim 为 `spring_structure_not_found`、live 有信号；下一步需对齐 live hub payload、sim parquet、decision audit 与结构扫描中间字段，先定性为数据/输入差异还是结构逻辑差异，再决定是否进入单问题 patch。
3. 本地 1m / idx 数据缺口实际修复：`MLNUSDT`、`SYSUSDT` parquet 缺尾，以及 `STARUSDT` 本地 contract parquet 有数据但目标窗口 `close_idx/high_idx/low_idx` 为 NaN。当前只完成取证 preflight fail-fast，尚未补齐历史 parquet / idx 或重跑受影响审计；该问题应与 live execution 归因、Spring 结构差异分开处理。
```
