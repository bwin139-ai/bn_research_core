...
8. 审计实盘延迟入场导致 SL 已不可提交的场景：以 `HUSDT 2026-04-30 18:15 BJ` 为样本，对比 C 点收盘价、`C+1m` 开盘价、signal payload current_price、实际 entry fill、resolved SL；评估是否在 live entry 前增加“entry price 已越过或过近 SL，则跳过入场”的 pre-submit guard。该 guard 目标是避免因 `C+1m` 后二十几秒入场延迟和市场波动导致保护单必然失败、随后应急平仓造成高概率亏损。
