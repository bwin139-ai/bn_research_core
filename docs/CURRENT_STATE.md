...
15. 2026-05-01 文档 checkpoint：下一刀只优化 Snapback live Telegram bot 消息的可读格式，不改变交易语义、下单顺序、state/audit 字段或执行风控。目标格式统一为多行消息头 `[HH:MM:SS 🦅 SNP] {account}`，其中时间取具体业务事件时间：signal 使用 `signal_time`，ENTRY/SL/TP 使用交易所订单创建/更新时间，离场使用 `trade_row.exit_time`。雷达、ENTRY、SL、TP、开仓、离场消息均保留原核心字段，但拆为多行展示；Snapback 使用 `SNP`，后续 Spring 若复用公共 BN_EXEC 通知头则应显示 `SPR`。
