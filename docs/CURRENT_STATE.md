...
10. 新增 `strategies/tvr/config.live_trader.smoke_10u.json` 与 `strategies/tvr/live_trader.py`：
   ...
   - open trade reconcile 会查询 TP 订单与 LONG position；TP 成交时清理 state 并输出 EXIT，position 已关闭但 TP 未成交时按外部 `POSITION_CLOSED` 清理并取消孤儿 TP，position 仍存在但 TP 查询失败/终态未成交时 fail-fast 并推送 CRITICAL。
...
- live_trader.execution.max_entry_notional_usdt = CLUSDT:11, BZUSDT:11, XAUUSDT:11, XAGUSDT:11
...
TVR recovery ladder 仍是计划语义，当前 live 未实现自动 recovery；后续实现必须先升级为多 lot state，并用 max_symbol_entry_notional_usdt 按本金成本口径限制每 symbol 最大投入。
```
