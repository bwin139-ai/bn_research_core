...
TVR recovery ladder 已进入配置/校验预备阶段，但当前 live 仍未实现自动 recovery；`recovery.enabled=true` 会 fail-fast，后续实现必须先升级为多 lot state，并用 max_symbol_entry_notional_usdt 按本金成本口径限制每 symbol 最大投入。
```
