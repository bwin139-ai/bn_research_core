...
5. 当前事实流包括：
   ...
   - rolling 24h stats：来自 TVR 自有增量 price history raw store，按 decision window 计算 `min/max/mean/median/p1/p5/p10/p20/latest`。
...
8. 新增 `strategies/tvr/config.decision_audit.json` 与 `strategies/tvr/decision_audit.py`：
   ...
   - 校验 data_hub 新鲜度、account、symbol 覆盖和字段可读性。
   - 应用 JSON `tradable_symbols` 白名单、`history_sufficient`、`funding_rate_entry_max`、`entry_drop_pct` 和配置化 notional cap。
...
10. 新增 `strategies/tvr/config.live_trader.smoke_10u.json` 与 `strategies/tvr/live_trader.py`：
   - 读取最新 TVR decision audit selected intents，不重新计算信号。
   - 显式 `allow_live_order=true` 才允许真实下单。
   - 入场使用 `LIMIT + GTX` post-only maker BUY。
...
- decision_audit.decision.entry_drop_pct = 0.01
- decision_audit.decision.funding_rate_entry_max = 0.0001
- decision_audit.decision.take_profit_pct = 0.01
- decision_audit.risk.proposed_order_notional_usdt = 10
- decision_audit.risk.max_symbol_notional_usdt = 10
- decision_audit.risk.max_total_notional_usdt = 40
- live_trader.allow_live_order = true
- live_trader.execution.order_notional_usdt = 10
- live_trader.execution.max_entry_notional_usdt = 11
- live_trader.execution.max_open_trades = 1
- live_trader.execution.leverage = 1
- live_trader.execution.post_only_time_in_force = GTX
- live_trader.execution.entry_price_offset_pct = 0.0005
- live_trader.execution.take_profit_pct = 0.01
```
...
TVR decision audit 的可交易品种由 JSON `tradable_symbols` 显式白名单决定；DataHub 仍采集完整 TradFi universe。
...
TVR 当前提交版 live_trader 尚未按 p1/p5/p10/p20 分位触发入场；后续实盘逻辑 patch 必须改为 current_24h_return <= selected_percentile_return 后按盘口 maker 入场。
```

当前 pending：

1. 第一刀：实现 TVR research history store / history backfill，对所有 TradFi 品种尽量补齐上市以来完整 1m contract kline，必须支持 cursor 断点续传、Gateway quota gate 和审计落盘。
2. 第二刀：实现 TVR percentile reclaim backtest，读取 research history store，评估 p1/p5/p10/p20 触发后 TP 0.5% / 1% 的命中率、最短/最长/平均/中位耗时和未命中比例。
3. 第三刀：修正 TVR decision_audit / live_trader 实盘入场逻辑，废除当前 `entry_drop_pct + latest` 口径，改为 `current_24h_return <= selected_percentile_return`，触发后按实时盘口 best bid 侧 `BUY LIMIT GTX`。
4. 后续可在服务器常驻启动 TVR data_hub loop；price history sync 在 loop 中持续开启，并按 cursor state 增量补齐，不应每次启动重拉完整决策窗口。
5. 若 Binance exchangeInfo 的 TradFi 分类字段继续变化，先以落盘事实修正 TVR universe 识别语义，不得硬编码品种兜底。
6. TVR live trader 可先用 `--dry-run` 在服务器读取真实 decision audit 验证，再用小资金账户启动 `config.live_trader.smoke_10u.json` 做 maker-only live smoke。
