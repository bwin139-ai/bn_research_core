...
- live_trader.execution.leverage = 5
...
- live_trader.recovery.enabled = false
- live_trader.recovery.anchor = HIGHEST_OPEN_ENTRY
- live_trader.recovery.grid_step_pct = 0.05
- live_trader.recovery.min_spacing_hours = 24
...
TVR recovery ladder 已进入配置/校验与 state 预备阶段，但当前 live 仍未实现自动 recovery；`recovery.enabled=true` 会 fail-fast。当前 live 建立单笔 `open_trade` 时会同步写入 `open_lots[0]` 镜像，旧 state 若只有 `open_trade` 会在 reconcile 时补齐镜像；后续仍需实现真正多 lot lifecycle，并用 max_symbol_entry_notional_usdt 按本金成本口径限制每 symbol 最大投入。
```
...
19. 2026-05-11 已将 TVR recovery ladder 计划落入 `docs/TVR项目语义基线.md`：每 symbol 新增计划字段 `max_symbol_entry_notional_usdt`，按 open lots 的 entry_notional 本金成本限制最大投入；recovery 锚点计划使用最高价未平 lot，`grid_step_pct=5%` 时三层门槛形如 `100 -> 95 -> 90`，并要求 `min_spacing_hours` 防止快速用尽层数。该能力尚未进入 live 代码。
...
21. 2026-05-11 TVR recovery 第一刀进入代码：`config.live_trader.smoke_10u.json` 新增 `execution.max_symbol_entry_notional_usdt` 与 `recovery` 显式配置；`live_trader.load_config()` 会校验 symbol key、单笔 notional 不超过 symbol 本金上限、`anchor=HIGHEST_OPEN_ENTRY`，且在多 lot recovery 实现前若 `recovery.enabled=true` 会 fail-fast。本刀不改变实盘入场/退出行为。
