...
39. 2026-06-27 Telegram 控制面韧性第二层开始推进：新增 `TG_PROXY_URLS` 多代理配置语义，逗号或空白分隔并按顺序优先，旧 `TG_PROXY_URL` 仍作为兼容追加项。`core/notify/tg_queue_sender.py` 每条消息发送时会按代理顺序 failover；`run_manual_trade_bot.py` 启动时调用 Telegram `getMe` 探测代理并选择第一个健康代理用于 polling；`core/process_monitor.py` 的 `telegram_bot_api` 检查改为任一代理成功即健康，全部代理失败才告警。2026-06-28 已补上 manual bot polling 层代理自恢复：当 polling 层出现 Telegram `NetworkError` / `TimedOut` 且配置了多个代理时，bot 进程会原地 re-exec 并重新探测代理列表；该机制只处理 `update is None` 的 polling 网络错误，不处理用户命令执行过程中的业务异常。
40. 2026-06-28 MacBook 本地代理混用风险已固定为工具脚本：`tools/mac_proxy/proxy_status.sh`、`tools/mac_proxy/use_aws_ssh_socks.sh`、`tools/mac_proxy/use_aws_wireguard_direct.sh`、`tools/mac_proxy/use_monoproxy.sh`。AWS SSH SOCKS 模式启动本机 SSH SOCKS 隧道 `127.0.0.1:18080 -> ubuntu@13.230.97.189`，并同步 macOS Wi-Fi SOCKS、git 全局代理与 `~/.zshrc` 托管代理块；MonoProxy 模式恢复 `127.0.0.1:8118/8119`；AWS WireGuard direct 模式关闭 macOS HTTP/HTTPS/SOCKS 系统代理、删除 git 全局代理、清理新 shell proxy env，并停止本脚本持有的 AWS SSH SOCKS listener。脚本不保存私钥内容，只引用本机 key 路径；该工具只用于 MacBook 开发环境，不得复制到阿里云生产交易进程环境。
41. 2026-06-28 AWS Lightsail Tokyo WireGuard / Codex Desktop 断流排查事实：MacBook / iPhone 通过 WireGuard 访问普通网站和 ChatGPT 网页流畅，但 Codex Desktop 经常在 `https://chatgpt.com/backend-api/codex/responses` 报 `stream disconnected before completion`。AWS 服务器只读检查显示 `wg0` MTU 为 `1280`，IPv4 forwarding 开启，UFW 放行 `wg0 <-> ens5` 转发，NAT `MASQUERADE` 已存在；初查时 `iptables -t mangle` 为空，服务器存在 IPv6 地址且未禁用 IPv6，WireGuard 客户端当前只配置 IPv4 `10.89.0.0/24`。同日已在 AWS 运行时与 `/etc/wireguard/wg0.conf` 持久化补上 `wg0 -> ens5` TCP SYN 的 `TCPMSS --clamp-mss-to-pmtu`，该规则只影响 WireGuard client 经 AWS 出口访问外网的新建 TCP 连接，不改变 Telegram tinyproxy、阿里云生产交易进程或 Binance API 出口。当前下一步是用 `use_aws_wireguard_direct.sh` 排除 MacBook 残留系统/git/shell 代理与 IPv6 泄漏后复测 Codex Desktop。
42. 2026-06-28 进一步确认 Codex Desktop 同一断流报错也曾在 MonoProxy 下出现；用户切换 MonoProxy server 后当前聊天恢复。因此问题不再能单独归因于 AWS WireGuard，必须按 Codex Desktop 长连接 / 代理节点 / IPv4-IPv6 出口 / 本机 DNS 与进程继承环境共同排查。新增只读探针 `tools/mac_proxy/probe_codex_network.sh`：显式清理自身 curl inherited proxy env，分别测试 direct、MonoProxy HTTP、MonoProxy SOCKS、AWS SOCKS 到 `chatgpt.com/cdn-cgi/trace` 与 `https://chatgpt.com/backend-api/codex/responses` 的 HTTP code、HTTP version、耗时和出口 trace。当前现场事实：macOS `scutil --proxy` 为空、`scutil --dns` 返回 `No DNS configuration available`、git 全局代理为空，但当前 Codex 子进程 shell env 仍有旧 `ALL_PROXY=socks5h://127.0.0.1:18080`；MonoProxy 监听 `127.0.0.1:8118/8119`，非沙箱真实探针显示当前 MonoProxy 新节点可访问 ChatGPT，trace 为 `colo=NRT`、`http=http/2`、出口 IP 为 IPv6。
43. 2026-06-28 MacBook 代理操作收敛为 A/B/C 三档脚本：`tools/mac_proxy/use_mode_a_monoproxy.sh`、`tools/mac_proxy/use_mode_b_aws_wireguard.sh`、`tools/mac_proxy/use_mode_c_direct.sh`。脚本不控制 MonoProxy / WireGuard GUI App，只在用户手动开关 App 后校准和验证 macOS 系统代理、git proxy、shell proxy 与出口状态。A 要求 MonoProxy `8118/8119` 正在监听且 WireGuard 未激活；B 要求 WireGuard 出现 `10.89.0.x` 且 MonoProxy 已退出，并验证 direct public IPv4 为 `13.230.97.189`；C 要求 MonoProxy 与 WireGuard 都关闭，并验证普通直连网络可达。`proxy_status.sh` 会先输出 `Mode A/B/C: PASS/FAIL`，再展开系统代理、git proxy、shell proxy、listener、WireGuard、出口 IP 与 DNS。`~/.zshrc` 的 proxy env 以后只由 `bn_research_core proxy mode` 托管块维护；切换脚本会移除托管块外的旧 proxy export。
44. 2026-06-28 AWS Lightsail Tokyo SSH 安全策略已收紧：当前确认可通过 `ubuntu@13.230.97.189` + Lightsail SSH key 登录，`ubuntu` 属于 `sudo` 组；`sshd -T` 有效配置为 `PubkeyAuthentication yes`、`PasswordAuthentication no`、`KbdInteractiveAuthentication no`、`PermitRootLogin no`。该配置关闭密码登录与 root 直接 SSH 登录，但保留 `ubuntu` key 登录作为管理员入口。变更文件为服务器 `/etc/ssh/sshd_config.d/99-disable-root-login.conf`，修改后已执行 `sshd -t` 与 `systemctl reload ssh`，并用全新 SSH 会话复验登录成功。
45. 2026-06-29 AWS Lightsail Tokyo WireGuard `51820/udp` 出现 MacBook / iPhone 均无法联网现场：AWS `wg0`、IPv4 forwarding、NAT、UFW route、MSS clamp 与实例外网出口均正常，但客户端连 `13.230.97.189:51820` 时服务器只看到握手初始化包和响应包，`latest handshake` 不刷新，NAT 转发计数不增长。已保留原 `51820/udp`，新增 `443/udp` 备用入口：Lightsail IPv4 firewall 放行 `UDP 443`，服务器 UFW 放行 `443/udp`，并在 `/etc/wireguard/wg0.conf` 持久化 `iptables -t nat PREROUTING -i ens5 -p udp --dport 443 -j REDIRECT --to-ports 51820`。iPhone endpoint 改为 `13.230.97.189:443` 后一度恢复联网。
46. 2026-06-30 AWS WireGuard `443/udp` 再次出现 MacBook / iPhone 均不可用现场：服务器仍能在 `ens5 udp port 443` 上看到客户端 handshake initiation 并发出 response，但对应 peer 的 `latest handshake` 不刷新，NAT MASQUERADE 计数不增长；新建 iPhone peer `10.89.0.4/32` 后现象不变。因此当前事实不支持继续把问题归因于单一旧客户端配置，也不建议继续把 WireGuard UDP 端口试错作为主线。已验证 AWS SSH SOCKS TCP 通路 `127.0.0.1:18080 -> ubuntu@13.230.97.189` 连续 5 轮访问 `chatgpt.com/cdn-cgi/trace` 与 `https://chatgpt.com/backend-api/codex/responses` 成功，Codex endpoint 返回预期 `405`。`tools/mac_proxy` 新增正式 D 模式 `use_mode_d_aws_ssh_socks.sh`，`proxy_status.sh` 增加 `Mode D AWS SSH SOCKS` 判定；后续稳定性主线改为 AWS 私有 TCP/SOCKS，WireGuard 作为备用。
47. 2026-06-30 AWS Lightsail Tokyo 新增 iPhone E 模式：安装 `shadowsocks-libev`，新增 systemd 托管的 Shadowsocks TCP 服务，监听 `0.0.0.0:443/tcp`，method 为 `chacha20-ietf-poly1305`，服务进程以 `nobody:nogroup` 运行，配置文件为 `/etc/shadowsocks-libev/config.json` 且不写入仓库。Lightsail 外层 firewall 与 UFW 均放行 `443/tcp`。该服务供 iPhone `Outline App` 测试 AWS 私有 TCP 通路，不影响 MacBook 当前 MonoProxy A 模式、AWS tinyproxy `80/tcp`、WireGuard `51820/udp` / `443/udp` 或阿里云交易服务器。AWS 本机 `ss-local -> ss-server -> https://ifconfig.me/ip` 闭环返回 `13.230.97.189`，Codex endpoint 返回预期 `405`。用户侧 iPhone 已导入 `AWS Tokyo E Outline` 并连接成功，手机访问 Google / YouTube 正常，公网出口查询显示 `13.230.97.189`、日本。
48. 2026-06-30 MacBook E 通道只读探针成功：在不切换 macOS 系统代理、不影响当前 Codex A 模式的前提下，通过 Homebrew 安装 `shadowsocks-libev`，临时启动本机 `ss-local` 监听 `127.0.0.1:18081` 并连接 AWS Shadowsocks `13.230.97.189:443/tcp`；连续 5 轮访问 `chatgpt.com/cdn-cgi/trace` 均返回 `http=200`、出口 IP `13.230.97.189`、`colo=NRT`，Codex endpoint 均返回预期 `405`。`tools/mac_proxy` 新增正式 E 模式 `use_mode_e_aws_outline.sh`，脚本从 AWS 读取 Shadowsocks 服务端配置并写入本机私有配置 `~/.config/bn_research_core/aws_outline_e_macbook.json`，不把 access key / password 写入仓库；`proxy_status.sh` 与 `probe_codex_network.sh` 已支持 `aws-outline` 检测。
49. 2026-06-30 MacBook 手动切换 D 模式失败现场：用户 Quit MonoProxy 后运行 `tools/mac_proxy/use_mode_d_aws_ssh_socks.sh`，`ssh` 报 `Load key "/Users/lyqmac/Downloads/LightsailDefaultKey-ap-northeast-1.pem": Operation not permitted`，随后 `ubuntu@13.230.97.189: Permission denied (publickey)`；`proxy_status.sh` 显示系统代理已为 direct/off、shell proxy 为空、MonoProxy listener 不存在，但 git global proxy 仍残留 `127.0.0.1:8118`，D/E listener 均未启动。现场元数据确认 Downloads 内 Lightsail key 权限为 `600`，但带有 macOS `com.apple.macl` / `quarantine` 等扩展属性；因此该失败是本机 key 读取权限/隔离问题，不是 AWS SSH 服务不可用。`tools/mac_proxy` 已新增 `install_aws_lightsail_key.sh`，并将 D/E 默认 key 优先路径改为 `~/.ssh/aws_lightsail_tokyo.pem`，脚本会在启动隧道前验证 key 可读并给出明确修复提示。
50. 2026-06-30 MacBook D 模式第二次实测：安装 key 后 `use_mode_d_aws_ssh_socks.sh` 可启动 SSH SOCKS listener `127.0.0.1:18080`，系统 SOCKS 与 git proxy 均切到 `18080`，浏览器访问 Google / YouTube / ChatGPT 可用，`AWS SSH SOCKS Codex endpoint` 返回预期 `405`；但 Codex Desktop 仍不可用。`proxy_status.sh` 总判定 `Mode D` 显示 FAIL 的直接原因是当前 terminal 未 `source ~/.zshrc`，shell proxy env 为空；这不影响 macOS 系统代理，但会造成严格状态判定失败。综合 MonoProxy 可用事实，当前判断 Codex Desktop 对 HTTP/HTTPS 代理比对 SOCKS 更可靠。D 模式已升级为 AWS SSH HTTP+SOCKS 双入口：`127.0.0.1:18082 -> SSH local forward -> AWS tinyproxy 127.0.0.1:80` 用于 macOS HTTP/HTTPS、git 和 shell HTTP proxy，`127.0.0.1:18080` 保留为 SSH SOCKS。
51. 2026-06-30 MacBook D 模式第三次实测：D 双入口配置可达成 `Mode D AWS SSH HTTP+SOCKS: PASS`，系统代理、git proxy、shell proxy、`18082/18080` listener 均匹配；但实际浏览器访问 Google / YouTube / Binance / ChatGPT 全部不可用，`proxy_status.sh` quick checks 对 AWS SSH HTTP/SOCKS 出现 timeout。事实复核：AWS 服务器本机 direct 与 tinyproxy 访问 Google / YouTube / Binance / ChatGPT 均秒级成功；MacBook 在不切系统代理时临时启动相同 SSH HTTP+SOCKS 隧道并连续 5 轮探测 ChatGPT trace / Codex endpoint / SOCKS trace 全部成功。因此当前更支持“macOS 系统代理同时启用 HTTP 与 SOCKS 后应用选路/本机行为异常”，而不是 AWS 服务端或 SSH 转发本体故障。D 模式已进一步改为系统 HTTP-only：macOS Wi-Fi 只启用 HTTP/HTTPS `127.0.0.1:18082`，系统 SOCKS 关闭；`127.0.0.1:18080` 仅保留为手动 SOCKS 探针，shell 不再导出 `ALL_PROXY`。
52. 2026-06-30 MacBook D 模式第四次实测成功：用户 Quit MonoProxy、确认 WireGuard 关闭后运行 `tools/mac_proxy/use_mode_d_aws_ssh_socks.sh`、`source ~/.zshrc`、`tools/mac_proxy/proxy_status.sh`，状态显示 `Mode D AWS SSH HTTP: PASS`。macOS Wi-Fi 仅启用 HTTP/HTTPS `127.0.0.1:18082`，SOCKS 为 `No`；git proxy 与 shell `http_proxy/https_proxy` 均指向 `http://127.0.0.1:18082`，`ALL_PROXY` 不再设置；本机 `ssh` 同时监听 `18082` 与 `18080`。quick checks 显示 AWS SSH HTTP 出口可达，Codex endpoint 返回预期 `405`。用户随后确认“本消息是在通道 D 发送”，说明 D HTTP-only 模式已实际承载 Codex Desktop 当前聊天。终端里 `zsh: command not found: #` 只是把说明行 `# Quit MonoProxy...` 粘进 zsh 后产生的无害提示，不影响切换结果。
53. 2026-06-30 MacBook D 模式后续复测再次出现无法联网，而 iPhone E 通道保持正常。基于 iPhone E 已稳定连接 AWS Outline/Shadowsocks `13.230.97.189:443/tcp`，MacBook 改测 E 模式：用户 Quit MonoProxy、确认 WireGuard 关闭后运行 `tools/mac_proxy/use_mode_e_aws_outline.sh`、`source ~/.zshrc`、`tools/mac_proxy/proxy_status.sh`，状态显示 `Mode E AWS Outline/Shadowsocks: PASS`。macOS Wi-Fi HTTP/HTTPS 均关闭，SOCKS 指向 `127.0.0.1:18081`；git proxy 与 shell `all_proxy/ALL_PROXY` 均指向 `socks5h://127.0.0.1:18081`；本机 `ss-local` 监听 `18081`，AWS Outline/Shadowsocks 出口 IP 为 `13.230.97.189`，Codex endpoint 返回预期 `405`。用户随后确认“本消息是在 MacBook E 通道发送”，说明 E 模式已实际承载 Codex Desktop 当前聊天。当前个人代理稳定性观察主线调整为 iPhone E + MacBook E 连续观察 24 小时；A / MonoProxy 保留为保底回退，D / AWS SSH HTTP 降为调试备用，B / WireGuard UDP 降为实验备用。
54. 2026-07-01 新增 AWS 私有代理强度对照：MacBook E SOCKS-only 能承载当前轻量 Codex 线程，但在另一个持续时间较久、上下文很长且已有多文件编辑状态的“实现核心资产策略”线程中，E 下发送消息失败，报 `failed to send websocket request: IO error: Broken pipe` 与 `stream disconnected before completion`；切回 A / MonoProxy 后同一长线程交互正常。因此当前事实表明 E SOCKS-only 尚不能完全平替 A。`tools/mac_proxy` 新增 E+ 模式 `use_mode_e_aws_outline_http.sh`：底层仍使用 AWS Outline/Shadowsocks `13.230.97.189:443/tcp` 与本机 `ss-local 127.0.0.1:18081`，但额外通过本机 `privoxy 127.0.0.1:18083` 提供 HTTP/HTTPS proxy；macOS/git/shell 均指向 HTTP 入口，用于对标 MonoProxy 的 HTTP 代理行为并复测 Codex 长线程、WebSocket、streaming 稳定性。本机已通过 Homebrew 安装 `privoxy`，当前路径为 `/usr/local/sbin/privoxy`。
55. 2026-07-01 E+ 模式长线程实测成功：MacBook 运行 `tools/mac_proxy/use_mode_e_aws_outline_http.sh` 后，`proxy_status.sh` 显示 `Mode E+ AWS Outline HTTP: PASS`，HTTP/HTTPS 系统代理、git proxy 与 shell proxy 均指向 `127.0.0.1:18083`，本机 `ss-local 18081` 与 `privoxy 18083` 均正常监听，AWS Outline HTTP 出口为 `13.230.97.189`，Codex endpoint 返回预期 `405`。同一个此前在 E SOCKS-only 下失败、在 A / MonoProxy 下正常的“实现核心资产策略”长线程，在 E+ 下成功完成交互并返回阿里云策略进程检查结果。因此当前主力候选从 E SOCKS-only 升级为 E+ / AWS Outline HTTP；A 继续作为保底，E SOCKS-only 作为轻量备用，D / SSH HTTP 与 B / WireGuard 降为调试/实验备用。
56. 2026-07-01 E+ 模式继续暴露长线程历史分页边界：MacBook 在 E+ 下浏览网页和 YouTube 流畅，`proxy_status.sh` 真实环境显示 `Mode E+ AWS Outline HTTP: PASS`，多轮 `probe_codex_network.sh` 对 `aws-outline` 与 `aws-outline-http` 均显示 Cloudflare `colo=NRT`、HTTP/2、Codex endpoint `405`、单次约 `0.3-0.5s`；但用户在某个 Codex 长线程中向上滚动时只能加载最近聊天记录，A / MonoProxy 下可加载完整早期记录。为增强 E+ 对长连接/大响应/历史分页的适配，`privoxy` 生成配置已调整为中性代理 `toggle 0`、`keep-alive-timeout 300`、`tolerate-pipelining 1`、`socket-timeout 600`，并打开轻量 `debug 1/2/8192` 日志，便于后续复现时检查本机 HTTP CONNECT 层是否发生 timeout 或连接关闭。
57. 2026-07-01 E+ 历史分页复现后的进一步归因：`privoxy` 日志显示 `chatgpt.com` / `ab.chatgpt.com` 连接上出现 `client socket has become unusable while the server socket is still open`，且 `ab.chatgpt.com` 有一次关闭时 `Data available: 1`；这表明当前 E+ 的 HTTP CONNECT 链路能建立，但 Codex 客户端侧存在提前关闭本机代理连接的现象。对比 A / MonoProxy 形态后，新的主要差异从 keep-alive 转为“E+ 只开 HTTP、未开系统 SOCKS，而 A 同时开 HTTP 8118 + SOCKS 8119”。因此 E+ 已调整为双入口模式：macOS HTTP/HTTPS 继续走 `127.0.0.1:18083`，系统 SOCKS 同时启用 `127.0.0.1:18081`，shell env 也同时保留 HTTP 与 `all_proxy=socks5h://127.0.0.1:18081`，用于更接近 MonoProxy 行为并复测 Codex 长线程历史分页。
58. 2026-07-01 E+ 双入口复测成功：用户在同一个此前只能上滑一两页、缺少左侧虚拟滚动进度条的“实现核心资产策略”长线程中复测，切换到双入口 E+ 后已能刷到线程最开头记录，左侧滚动进度条恢复显示。因此当前事实链闭环为：E SOCKS-only 可承载轻量线程但长线程发送失败；E+ HTTP-only 可改善长线程发送但仍不能完整加载早期历史分页；E+ HTTP+SOCKS 双入口最接近 A / MonoProxy 形态，并已恢复 Codex 长线程历史分页。当前 MacBook 主力模式应使用双入口 E+，A / MonoProxy 仍保留为保底回退。
59. 2026-07-01 E+ 继续暴露“多长线程历史分页”干扰后已补强本机 proxy bypass：`privoxy` 日志显示 E+ 能建立 `chatgpt.com:443` CONNECT，但同一时间大量 Apple/iCloud 中国、Google、VSCode、微信、MetaMask 等后台连接也进入 AWS Outline 链路，并频繁出现客户端侧连接关闭记录。对比 A / MonoProxy 的系统例外形态后，`tools/mac_proxy` 已在 E / E+ 切换时写入标准 macOS proxy bypass domains，使局域网、Apple/iCloud 中国、微信/QQ 等更适合直连的请求绕过 AWS Outline，减少与 Codex 长线程历史分页争用同一条代理链路。用户重新运行 E+ 脚本后确认此前两个长线程都恢复左侧进度条并能刷出全部聊天历史。因此当前 MacBook 主力模式仍为 E+ HTTP+SOCKS 双入口，并要求保留该 bypass 配置；若后续新增长线程加载异常，先复查 `tools/mac_proxy/proxy_status.sh` 与 E+ bypass 是否仍生效，再考虑引入按 App 分流工具。
...
2026-07-01 CAL live trader 增强手动平仓兼容：当 CAL 持有本地 open lots 且对应 TP 订单变为 `CANCELED/EXPIRED/REJECTED` 等非成交终态时，不再立即视为不可恢复异常；策略会先查询交易所 LONG 仓位，若该 symbol 交易所仓位已经为 0，则判定为外部/手动平仓完成，将本地 open lots 以 `POSITION_CLOSED_EXTERNAL` 写入 closed lots、清空该 symbol 的 open lots 与 repeat counts，并清理 `tp_order_terminal_not_filled` 暂停标记，使该品种后续可按当前配置重新进入 CAL ladder。若 TP 非成交终态但交易所仍有 LONG 仓位，则仍按原保护逻辑暂停。相同 symbol/相同 reason 的重复暂停不再每轮重复推送 Telegram，避免 10 秒循环刷屏。
...
2026-06-28 Telegram 控制面代理冗余进入生产状态并完成 AWS 静态 IP 绑定：阿里云交易服务器 `.env` / `deploy.env` 已配置 `TG_PROXY_URLS=http://13.230.97.189:80,http://206.189.90.153:8888`，AWS Lightsail Tokyo Static IP `13.230.97.189:80`（名称 `proxy-toyko`）为首选 Telegram Bot API tinyproxy，旧 DigitalOcean `206.189.90.153:8888` 为备用；旧兼容变量 `TG_PROXY_URL=http://206.189.90.153:8888` 仍保留。AWS Tokyo tinyproxy 只允许 `127.0.0.1` 与阿里云出口 IP `8.218.96.252`，并只用于 Telegram 请求；Binance API 必须继续从阿里云直连，生产环境不得设置 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 等全局代理变量。AWS Tokyo 同机 WireGuard 已作为个人备用网络测试通过，MacBook 与 iPhone 均能以 `13.230.97.189` 作为出口访问外网；客户端配置和二维码已在 `/private/tmp/personal-proxy-tokyo-test/` 重新生成。DigitalOcean SGP1 个人 WireGuard 节点 `139.59.116.55` 因性能过慢已销毁，不是活跃节点。线上 `run_manual_trade_bot.py`、`core/notify/tg_queue_sender.py`、`core/process_monitor.py` 均恢复为各 1 个进程，`process_monitor` 连续记录 `status=ok`，sender 日志显示消息经 `http://13.230.97.189:80` 发送成功。2026-06-28 AWS 从临时公网 IP 切换到 Static IP 后，暴露出 manual bot polling 运行中不会自动切换第二代理的问题；已在 `core/manual_trade_bot.py` 增加 polling 层 Telegram 网络错误 re-exec 自恢复，重启后会重新按 `TG_PROXY_URLS` 健康探测选择代理。只要该 Static IP 保持 attached 且未 release，实例 stop/start 后公网入口也应保持不变；若 detach/release 或重建代理机，必须同步更新代理配置与 WireGuard 客户端配置。
