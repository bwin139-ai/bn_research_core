...
8. `/trade close` 默认使用 `/set s` 维护的当前 symbol，也支持在 action 后显式携带 `SYMBOL`；支持 `/trade close [SYMBOL] ACCOUNT[ | ACCOUNT...] [PO] [PCT%]`、`/trade close [SYMBOL] ACCOUNT[ | ACCOUNT...] M|PO [PCT%]` 与 `/trade close [SYMBOL] ACCOUNT[ | ACCOUNT...] L PRICE [PCT%]` 命令式 LONG 平仓入口；省略 `M/PO/L` 时默认 `PO`：
...
11. `/trade sl [SYMBOL] ACCOUNT[ | ACCOUNT...] PRICE [PCT%]` 命令式 LONG 止损挂单入口：默认使用当前 symbol，也可显式携带 symbol。
