...
- decision_audit.decision.entry_percentile = p10
...
3. Snapback live 的同类 finalized payload wait 失败也接入同一公共观测落盘与 60 分钟摘要 helper；本刀不改变 Snapback 私有 wait 判定逻辑。
4. 三套策略会将该事件落盘到：
...
1. Snapback live 的 wait / mismatch 代码仍在 `strategies/snapback/run_live.py` 私有实现内；后续可进一步收敛到公共 `core/live/live_data_gate.py` wait helper。
