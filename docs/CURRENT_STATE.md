...
2026-06-28 Telegram 控制面代理冗余进入生产状态：阿里云交易服务器 `.env` / `deploy.env` 已配置 `TG_PROXY_URLS=http://54.95.135.134:80,http://206.189.90.153:8888`，AWS Lightsail Tokyo `54.95.135.134:80` 为首选 Telegram Bot API tinyproxy，旧 DigitalOcean `206.189.90.153:8888` 为备用；旧兼容变量 `TG_PROXY_URL=http://206.189.90.153:8888` 仍保留。AWS Tokyo tinyproxy 只允许 `127.0.0.1` 与阿里云出口 IP `8.218.96.252`，并只用于 Telegram 请求；Binance API 必须继续从阿里云直连，生产环境不得设置 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY` 等全局代理变量。AWS Tokyo 同机 WireGuard 已作为个人备用网络测试通过，MacBook 与 iPhone 均能以 `54.95.135.134` 作为出口访问外网；DigitalOcean SGP1 个人 WireGuard 节点 `139.59.116.55` 因性能过慢已销毁，不是活跃节点。13:15 后线上 `run_manual_trade_bot.py`、`core/notify/tg_queue_sender.py`、`core/process_monitor.py` 均恢复为各 1 个进程，`process_monitor` 连续记录 `status=ok`，sender 日志显示消息经 `http://54.95.135.134:80` 发送成功。AWS Lightsail 当前未绑定 Static IP；实例保持 running 或普通 reboot 时公网 IP 通常保持不变，但 stop/start 或重建可能变更，长期生产使用应绑定 Lightsail Static IP 并同步更新代理配置与 WireGuard 客户端配置。
