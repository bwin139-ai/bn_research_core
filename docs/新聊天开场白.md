...
14. 已完成对 `BEATUSDT` live-only 样本的初步排查，确认其 `vol_24h` 并未低于 universe 阈值，不能简单归因于 `24h_quote_volume_min` 过滤。
...
10. 已确认 `FARTCOINUSDT` 这笔共同样本的 structure 差异，不是 universe 差异，也不是下游投影写坏，而是 live 输入层在首轮取数时读取到了尚未最终固化的 closed bar。
11. 已确认在当前样本窗口中，`FARTCOINUSDT` 之外的其他 matched 样本未再暴露出同类 structure 分叉证据。
12. 已确认 `sim_signals` 与 `live_signals` 中 `current_price / c_price` 字段存在语义混写风险；其中 `sim` 最终成交价仍由 broker 按 `CB.open` 成交，不直接受 signal snapshot 中该字段歧义影响。
13. 已确认同一虚拟环境同时运行两个 live 进程在工程上可行，隔离前提是：不同账户、不同配置文件、不同日志文件。
...
5. 需要继续确认 `BEATUSDT` 这类 live-only 样本的缺失原因，判断它究竟停在 universe 层、输入层，还是仍有同类首轮取数过早问题。
...
```text
下一步顺序：
1. 先验证 live runner 调度点后移（第 2 秒取数）是否能改善刚闭合 bar 首轮未固化问题
2. 继续以最小样本补审方式确认 live-only / sim-only 差异究竟停在 universe、输入层还是 structure 层
3. 形成当前阶段结论归档（验证通过项 / 待改进项）
4. 继续决定 bn truth 是否增加条件委托 / algo 父单独立真相层
5. 视运行稳定性，启动双 live：高频审计 live + 盈利验证 live
```
