...
该模块不改变 LONG-only 策略基线，不提供 SHORT 执行能力。2026-05-28 起，账户级管理员门户允许读取手动 `hedge_short` 产生的 SHORT 历史事实；这只是只读审计与展示能力，不属于策略方向语义。
...
6. `positions`：由 `trades` 派生出的 LONG / 手动 `hedge_short` SHORT 仓位生命周期视图，对应 Binance App 的仓位历史。
...
`positions` 是派生视图，不替代交易所原始事实。正常行包含 `position_side=LONG` 的闭合仓位，以及账户级手动 `hedge_short` 产生的 `position_side=SHORT` 闭合仓位，状态为 `CLOSED`；若存在平仓成交但本地 trades 缺少对应开仓事实，必须写 `status=INCOMPLETE` 与 `incomplete_reason`，不得伪造 entry price / open time。
...
3. 每轮额外读取当前非零持仓与当前挂单，从中提取 `current_position_symbols` 与 `current_open_order_symbols`。
4. 本轮只对 `active_sync_symbols ∪ current_position_symbols ∪ current_open_order_symbols ∪ explicit_symbols` 拉取 `orders` / `trades`。
5. `state/exchange_history/{account}/symbols.json` 只作为历史出现过的 symbol 索引，只增量维护，不作为每轮 `orders` / `trades` 的 API 扫描输入。
6. 显式命令行 `--symbol` / `--symbol-file` 只作为人工补查入口，可临时追加到本轮同步 symbol 集合；日常 loop 不依赖它。

`income` 是账户级接口，不需要按 symbol 查询，覆盖系统内订单与系统外订单产生的账户资金事实。零成交的 `CANCELED` / `EXPIRED` 订单没有 income 事实，不纳入当前“完整交易账本”的完整性要求。部分传统金融映射品种存在 maker 零手续费；其开仓成交可能没有 realized PnL 与 commission income，因此必须依靠当前持仓 / 当前挂单 symbol 补充发现，不能只依赖 income。

全市场补齐不得作为日常 Telegram 查询路径。若需要全量历史，应走 Binance 异步导出接口并作为单独低频审计任务。

## 4. 增量规则

同步按 account / source / symbol / time window 执行；每轮执行顺序固定为先写入 `balance_snapshots`，再同步 `income/transfers`，再发现本轮同步 symbol 集合并同步 `orders/trades`，最后从已落盘 `trades` 派生 `positions`。

1. 默认窗口为最近 24 小时，只用于无成功 cursor 的首次窗口；已有成功 cursor 的 source / symbol 必须从 cursor overlap 继续，不得被默认窗口截断。
...
9. 写入前按 `dedupe_key` 去重；其中 `orders` 是订单状态快照，允许同一 `order_id` 后续从 `NEW` 更新为 `FILLED`，因此必须按 `dedupe_key` upsert 覆盖旧快照；`trades` / `income` / `transfers` 仍按不可变流水追加去重。
...
14. `positions` 只从已落盘 `trades` 派生，不推进交易所 cursor；LONG 使用 BUY 开仓、SELL 平仓，SHORT 使用 SELL 开仓、BUY 平仓。若派生发现非 LONG/SHORT 方向或无法解释的成交顺序冲突，必须返回非 ok。
15. `positions` 是派生视图，每轮必须基于当前已落盘 `trades` 重建，避免缺失 trades 回补后旧 `INCOMPLETE` 行继续残留。
16. 若 `positions` 派生出现 `INCOMPLETE`，且对应 symbol 属于本轮同步 symbol 集合，必须对该 account + symbol 从 `exchange_history_start_time` 起做一次定向 `orders` / `trades` 回补，再重新派生 `positions`。该回补只针对异常 symbol，不得退化为每轮全量 historical symbols 扫描。
...
正常视图展示 LONG 事实与账户级手动 `hedge_short` SHORT 历史事实，并必须显式标出 SHORT；除该管理员门户例外外，若交易所返回方向与策略 LONG-only 基线冲突，必须进入异常审计区，不得静默混入策略历史。
