...
from strategies.sweep_reclaim.live_execution import build_sweep_reclaim_live_execution_intent
...
def _build_run_id(account: str) -> str:
    ...
    return f"SWRLIVE_{account_key}_{ts_utc}"


def _projection_path(output_dir: str, run_id: str) -> Path:
    return Path(output_dir) / f"sweep_reclaim_live.{run_id}.jsonl"


def _heartbeat_path(output_dir: str, run_id: str) -> Path:
    return Path(output_dir) / f"sweep_reclaim_live_heartbeat.{run_id}.json"
...
def _validate_hub_payload(payload: Mapping[str, Any]) -> tuple[int, str, int, str, pd.DataFrame, dict[str, Any]]:
    ...
    if signal_time_ts != expected_signal_time_ts:
        raise ValueError(
            "signal_time_ts must equal latest_closed_bar_ts + 60000 for Sweep-Reclaim CB=C+1m semantics, "
...
def _audit_preview(audits: Mapping[str, Mapping[str, Any]], limit: int) -> list[dict[str, Any]]:
    ...
    for symbol in sorted(audits.keys()):
        ...
        rows.append({
            ...
            "h_time_ms": rec.get("h_time_ms"),
            "gamma_time_ms": rec.get("gamma_time_ms"),
            "b_time_ms": rec.get("b_time_ms"),
            "c_time_ms": rec.get("c_time_ms"),
            "bars_hb": rec.get("bars_hb"),
            "bc_bars": rec.get("bc_bars"),
            "hb_drop": rec.get("hb_drop"),
            "bc_rebound": rec.get("bc_rebound"),
            "bc_over_hb_bars": rec.get("bc_over_hb_bars"),
            "vol_climax": rec.get("vol_climax"),
        })
        if len(rows) >= limit:
            break
    return rows
...
def _not_ready_projection_row(
    *,
    ...
    row = {
        "schema_version": 1,
        "run_mode": "live",
        "projection_type": "sweep_reclaim_live",
        "strategy_name": "sweep-reclaim",
        "account": str(account).strip(),
        "run_id": run_id,
...
def run_once(
    *,
    ...
    if str(strategy_cfg.get("strategy_name") or "") != "sweep-reclaim":
        raise ValueError(f"config strategy_name must be sweep-reclaim, got {strategy_cfg.get('strategy_name')!r}")
    ...
    if execute_live:
        lifecycle_reconcile = reconcile_strategy_open_trades(
            ...
            source="sweep_reclaim_live_pre_scan",
            strategy_name="sweep-reclaim",
        )
        account_local_precheck = account_local_activity_precheck(account, strategy_name="sweep-reclaim")
    ...
    strategy = SweepReclaimStrategy(strategy_cfg)
    ...
    intent = build_sweep_reclaim_live_execution_intent(signal, account=account).to_dict() if signal else None
    local_state_snapshot = load_live_state(account, strategy_name="sweep-reclaim") if intent else None
    ...
    *,
    ...
    while True:
        if max_iterations > 0 and iteration >= max_iterations:
            ...
            logging.info("[Sweep-Reclaim-Live] loop completed | run_id=%s | iterations=%s", run_id, iteration)
            return
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Sweep-Reclaim live runner")
    parser.add_argument("--config", default="strategies/sweep_reclaim/config.json")
