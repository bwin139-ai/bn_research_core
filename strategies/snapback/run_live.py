...
def _next_signal_check_epoch(now_epoch: float | None = None) -> float:
    ...
    current_minute_second_second = now.replace(second=15, microsecond=0)
    if now < current_minute_second_second:
        return current_minute_second_second.timestamp()
    next_minute_second_second = now.replace(second=0, microsecond=0) + timedelta(minutes=1, seconds=15)
